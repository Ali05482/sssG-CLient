export const referralBuilder = (props,referral, doctor) => {
    const formattedDate = formatDateString(referral?.patientLastAccessed[0]?.createdAt);
    return `
    <style>
    table {
...
<table border="1" style="border-collapse:collapse;width: 100%;">
    <tbody>
        <tr>
            <td style="width: 50%;"><strong>Consultant to:&nbsp;</strong>${referral?.consultTo}</td>
            <td style="width: 50%;"><strong>Patient Last Assessed:&nbsp;</strong>${formattedDate=="Invalid date"?"":formattedDate}<br></td>
        </tr>
    </tbody>
</table>
<p><em><strong>PLEASE SEE THIS PATIENT IN CONSULTATION REGARDING:</strong></em></p>
