...
const SickNote = (props) => {
    ...
    const handleChange = (e) => {
        ...
        daysBetween.current.value = Math.floor((new Date(endDate.current.value) - new Date(startDate.current.value)) / (1000 * 60 * 60 * 24));
    };
...
<table border="1" style="border-collapse: collapse; width: 100%">
  <tbody>
    <tr>
      <td style="width: 50%"><strong>Name</strong>: ${props?.patient?.firstName + " " + props?.patient?.lastName || ''}</td>
      <td style="width: 50%"><strong>Date</strong>: ${new Date().toISOString().substr(0, 10)}</td>
    </tr>
    <tr>
      <td style="width: 50%"><strong>Address</strong>: ${props?.patient?.address || ''}</td>
      <td style="width: 50%"><strong>Phone</strong>:${props?.patient?.phoneNumber || ''}</td>
    </tr>
  </tbody>
</table>
<p><br /></p>
...
<table border="1" style="border-collapse: collapse; width: 100%">
  <tbody>
    <tr>
      <td style="width: 50%"><strong>FROM</strong>: ${props?.sickNote?.startDate || ''}</td>
      <td style="width: 50%"><strong>TO</strong>: ${props?.sickNote?.endDate || ''}<br /></td>
    </tr>
  </tbody>
</table>
<p><em>Comments:</em></p>
  ...
  <tbody>
    <tr>
      <td style="width: 33.3333%">
        <strong
          >Dr. ${props?.doctor?.firstName + " " + props?.doctor?.lastName || ''}<br /><img
    ...
    return (
        <>
            <div className="container">
                ...
                <div className="form-group">
                    <label htmlFor=""><b>Days Between</b></label>
                    <input type="number" disabled ref={daysBetween}  className="form-control" />
                </div>
                <div className="form-group my-3">
                    <button onClick={handleSickNote} type='button' className="btn btn-primary">Create</button>
                </div>
            </div>
        </>
    )
